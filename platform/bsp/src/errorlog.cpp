...
#define ISTIMEAVAILABLE  // Assume we want RTC in Errorlog if it's available
...
#define FIRSTLOGSECTOR  500
#define SECONDLOGSECTOR 501
...
typedef struct timeStamp_t     /// Timestamp stored for every logging entry if RTC is available
{
  union   // assure 32 bit alignment!
  {
	...
	uint64_t dummy[1];          ///< dummy to assure 64 bit alignment
  };
} TimeStamp_t;
...
typedef struct errorLog_t   /// Structure which is stored in flash for logging purpose
{
  union
  {
    ...
    uint64_t dummy[5];                    ///< dummy to assure 64 bit alignment
  };
} ErrorLog_t;
...
static uint32_t formatLogSeg(uint32_t sector)
{
  ...
  if (isSuccess(xcode))
  {
     if (FLASH_SUCCESS != iap_eraseFlash ((void *)segStartAdresse, 1))
     {
         xcode = DataError;
     }
...
{ 
  ...
  HAL_StatusTypeDef stat = HAL_RTC_GetTime(&hrtc, &time, RTC_FORMAT_BIN);
  if (HAL_OK != stat)
  {
	  time.Hours   = 0;
	  time.Minutes = 0;
...
#ifdef ISTIMEAVAILABLE
    logStructure.timeStamp.seconds    = (uint8_t) time.Seconds;
    logStructure.timeStamp.minutes    = (uint8_t) time.Minutes;
    logStructure.timeStamp.hours      = (uint8_t) time.Hours;
    logStructure.timeStamp.dayOfMonth = (uint8_t) date.Date;
    logStructure.timeStamp.month      = (uint8_t) date.Month;
    logStructure.timeStamp.year       = (uint16_t)date.Year +2000;
    // Note: we have a year 2100 problem: RTC only supports year as byte,
    // therefore only values between 0..99 are supported. Assume we are in
    // 21 century....
#else
