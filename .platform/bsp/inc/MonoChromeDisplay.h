...
public:
    ...
    */
    ...
    {};
#endif
    MonoChromeDisplay(I2C_HandleTypeDef* ifc, uint8_t addr) : U(ifc, addr) {};
...
public:
      ...
      */
     void print(std::string src) {print(src.c_str());}
...
void MonoChromeDisplay<U>::showDemoText() {
    clrscr();
    println("TECHNISCHE INFORMATIK");
    println("HOCHSCHULE OSNSBRUECK");
...
template<typename U>
void MonoChromeDisplay<U>::println(std::string src) {
    print(src);
    newline();
}
...
template<typename U>
void MonoChromeDisplay<U>::println(const char* src) {
    print(src);
    newline();
}
...
void MonoChromeDisplay<U>::bitmap(const uint8_t* picture,
    ...
    else if(PixelStorage::RLEencoding == pixStorage)
    {
        ...
        uint8_t idByte      = picture[4];
        uint8_t packedByte  = picture[5];
        uint8_t specialByte = picture[6];

        maxx >>= 3;
        size_t maxBytes = maxx*maxy;
        size_t picIndex = 11;   // offset if picture data.
        size_t mapIndex = 0;
